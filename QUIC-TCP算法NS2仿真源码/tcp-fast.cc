...
#ifdef TCP_DELAY_BIND_ALL //TCP_DELAY_BIND_ALL defined.
  
#else /* ! TCP_DELAY_BIND_ALL */ //Inactive code:
...
//	bind("alpha_tuning_", &alpha_tuning_);
	bind("gamma_", &gamma_);
    bind("rho_",&rho_);
//	printf("TCP-Fast.CC: gamma_= %.5f, rho_ = %.4f\n",gamma_,rho_);
...
FastTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (delay_bind(varName, localName, "gamma_", &gamma_, tracer))
		return TCL_OK;
    if (delay_bind(varName, localName, "rho_", &rho_, tracer))
		return TCL_OK;
		     ...
		     cwnd_array_q >= beta_ * avgRTT_ )
                {
                        ...
                        if (rho_ == 9.0)  //FastTCP w(t+1) = w(t) - gamma*S;
                        {
						    target_cwnd = ( (1-gamma_) * cwnd + (old_cwnd * (baseRTT_/avgRTT_) + alpha_) * gamma_);
						    //cout<<"(1)target_cwnd = " <<target_cwnd<<endl;
						     //cout<<"(2)cwnd = " <<cwnd<<endl;
						    // cout<<"(3)old_cwnd = " <<old_cwnd<<endl;
						     //cout<<"(4)baseRTT_ = " <<baseRTT_<<endl;
						    //cout<<"(5)avgRTT_ = " <<avgRTT_<<endl;
						    
                        }
                        else if (rho_ == 0.0)
						{
                            //Rou = 0p0: w(t+1) = w(t) - gamma*(BaseRTT/RTT)*w(t)*S;
                            target_cwnd = ( (1-gamma_ *old_cwnd*(baseRTT_/avgRTT_)) * cwnd 
                            + (old_cwnd * (baseRTT_/avgRTT_) + alpha_) * gamma_*old_cwnd*(baseRTT_/avgRTT_));
                        }
                        else if (rho_  == 0.5)
						{
                            //Rou = 0p5:NewAlg: w(t+1) = w(t) - gamma*(BaseRTT/RTT)*S;
                            target_cwnd = ( (1-gamma_ *(baseRTT_/avgRTT_)) * cwnd 
                            + (old_cwnd * (baseRTT_/avgRTT_) + alpha_) * gamma_*(baseRTT_/avgRTT_));
                        }
                        else if (rho_  == 1.0)
                        {
                            //Rou = 1p0: w(t+1) = w(t) - gamma*(BaseRTT/RTT)*(S/w(t));
                            #if 1
                            target_cwnd = ( (1-(gamma_ /old_cwnd)*(baseRTT_/avgRTT_)) * cwnd 
                            + (old_cwnd * (baseRTT_/avgRTT_) + alpha_) * (gamma_/old_cwnd)*(baseRTT_/avgRTT_));
                            #else
                            DeltaCwnd = (gamma_*baseRTT_)/(avgRTT)*
                           #endif
                        }
                       else if (rho_>1 ||rho_<0)
                       {
                             printf("Tcp-Fast.CC: Parameter rho_ error!\n");
                             exit(0);
                      }
		    else
                      {
                      /* target_cwnd =  cwnd -gamma_*(baseRTT_/avgRTT_)  *  [old_cwnd^(-2*rho_+1)] *  [cwnd - old_cwnd * (baseRTT_/avgRTT_) - alpha_) ];

                      RTT_Q = (baseRTT_/avgRTT_); 
                      S =  [cwnd - old_cwnd *RTT_Q- alpha_) ]
                      target_cwnd =  cwnd -gamma_*RTT_Q *  [old_cwnd^(-2*rho_+1)] *  S;
					   */
		    RTT_Q = (baseRTT_/avgRTT_); 
                      S =  cwnd - old_cwnd *RTT_Q- alpha_ ;
                      target_cwnd =  cwnd -gamma_*RTT_Q * pow(old_cwnd, (1-2*rho_))*  S;
					  
					  //printf("rho = %.3; step Size = %.3f", rho_, gamma_);
                      
                      }
						//FILE *tmpFile = fopen("p.txt","a+");
						//fprintf(tmpFile,"%.8f  %.8f   %.8f   %.8f \n",old_cwnd)
			double gamma=gamma_;
			double alpha=alpha_;
			double beta=beta_;
...
FastTcpAgent::fast_est_update_avgrtt(double rtt)
{
	...
	{
		avgRTT = (avgRTT*(ScaleThresh-1) + rtt)/ScaleThresh;
...
FastTcpAgent::fast_cc(double rtt, double old_cwnd)
{
	...
	if ( avgRTT_ - baseRTT_ < mi_threshold_ && (fast_opts & FAST_ALLOW_MI) ) {
	    //printf("7.gamma_= %.14f\n",gamma_);
	    //printf("avgRTT_ = %.14f, baseRTT = %.14f, mi_threshold_ = %.14f, fast_opts = %d\n\n", avgRTT_ ,baseRTT_,mi_threshold_,fast_opts);
        //printf("avgRTT_ = %.14f\n", avgRTT_ );
        //printf("fast_opts = %d\n\n", fast_opts);
